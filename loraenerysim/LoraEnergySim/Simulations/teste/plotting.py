...
def plot_packet_energy(node_data):
    TE_J = node_data['TotalEnergy'].iloc[0] / 1000
    ep = TE_J / (node_data['TotalPackets'].iloc[0])
    print("Energy per packet: ", ep, "(J)")
    num_pacotes = np.arange(0, node_data['TotalPackets'].iloc[0])
...
def plot_bit_energy(node_data):
    eb = (node_data['TotalEnergy'].iloc[0]/ (node_data['TotalBytes'].iloc[0] * 8))/1000
    print("Energy Consumed to Send One Bit of Data: ", eb, " J") #mj to J
    num_bit = np.arange(0, node_data['TotalBytes'].iloc[0]*8) #Transform bytes to bits
...
file = "/home/marianna/LoRaEnergySim/results/True_True_0.1_cnst_num_bytes.p"
...
for idx_p, p in enumerate(payload_sizes):
    for idx, s in enumerate(sigmas):
        ...
        # print(node_data['TotalEnergy'], " ", node_data['TotalPackets'], " ", node_data['TxRxEnergy'], " ", node_data['EnergyValuePackets'])
        
        # print()
        
        print("Total Bytes Transmitted: ", node_data['TotalBytes'].iloc[0])
        print("Total Packets Transmitted: ", node_data['TotalPackets'].iloc[0])
        print("Total Energy Consumed:" , node_data['TotalEnergy'].iloc[0])
        print("Total Packets Collided:" , node_data['CollidedPackets'].iloc[0])

        plot_bit_energy(node_data)
