...
# Função para extração do log de entrada e saída de cada função
LOG_FILE = "func_logs.txt"

def log_tempo_coroutine(func):
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        self = args[0]
        node_id = getattr(self, 'id', 'unknown')
        sim_time = getattr(self.env, 'now', 'unknown')

        with open(LOG_FILE, "a") as f:
            f.write(f"[{time.strftime('%H:%M:%S')}] [Sim {sim_time}] [Node {node_id}] --> Entrando em {func.__qualname__}\n")

        resultado = func(*args, **kwargs)

        def generator_wrapper():
            entrada_real = time.time()
            for item in resultado:
                yield item
            saida_real = time.time()
            sim_time_end = getattr(self.env, 'now', 'unknown')
            with open(LOG_FILE, "a") as f:
                f.write(
                    f"[{time.strftime('%H:%M:%S')}] [Sim {sim_time_end}] [Node {node_id}] <-- Saindo de {func.__qualname__} "
                    f"(duração real: {saida_real - entrada_real:.4f}s)\n"
                )
...
class Node:
    ...
    def run(self):
        ...
        random_wait = MAX_DELAY_START_PER_NODE_MS
        ...
        while True:
            ...
            random_wait = MAX_DELAY_BEFORE_SLEEP_MS
            yield self.env.timeout(random_wait)

            yield self.env.process(self.sleep())
            self.out_of_sleep = self.env.now
            ...
            self.last_transmission_time = self.env.now
    ...
    def send_tx(self, packet: UplinkMessage) -> bool:
        with open("tempos{}.txt".format(num_locations), "a") as f:
            f.writelines("Hora de envio, nó {} e payload {}: {} \n".format(self.id, self.payload_size, self.env.now))
        self.packets_sent += 1
    ...
    def send_rx_ack(self, rec_window: int, packet: UplinkMessage, ack: bool):
        ...
        if not ack:
            ...
            if rec_window == 1:
                rx_time = LoRaPacket.time_on_air(self.payload_size, packet.lora_param)
                ...
                rx_time = LoRaPacket.time_on_air(self.payload_size, temp_lora_param)
    ...
    def sleep(self):
        ...
        if  PRINT_ENABLED:
            print('{}: START sleeping'.format(self.id))
        self.change_state(NodeState.SLEEP)
        # Calcula o tempo da próxima transmissão baseado na última hibernação e o tempo setado pro dispositivo ficar desativado
        next_tx_time = self.out_of_sleep + self.sleep_time
        # O tempo de sleep do dispositivo então será o tempo de próxima transmissão menos o tempo atual da simulação
        now = self.env.now
        sleep_duration = max(0, next_tx_time - now) #O dispositivo não dorme no caso da subtração dar negativa
        # print(next_tx_time, now, sleep_duration)
