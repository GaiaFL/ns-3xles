...
class LogShadow:
    ...
    def __init__(self, gamma=3.76, d0=1, std=7.8, Lpld0=128.95, GL=0):
        self.gamma = gamma
        self.d0 = d0
        self.std = std
        if self.std<0:
    ...
    def tp_to_rss(self, indoor: bool, tp_dBm: int, d: float):
        ...
        # print(Lpl2, self.GL + Lpl)
        # print("Shadow: ", Lpl)
        # print("Shadow2: ", tp_dBm + self.GL + Lpl)
        if Lpl <0:
            Lpl = 0
        return tp_dBm + self.GL + Lpl    
        # return tp_dBm + Lpl2
    
    def calcRxPower(self, indoor: bool, tp_dBm: int, d: float):
        Lpl2 = - self.std - 10 * self.gamma * np.log10(d / self.d0)
        return tp_dBm + Lpl2
