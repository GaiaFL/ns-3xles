...
class AirInterface:
    ...
    def sf_collision(p1: UplinkMessage, p2: UplinkMessage):
        ...
        if p1.lora_param.sf == p2.lora_param.sf:
            ...
            with open("colisao.txt", "a") as f:
                f.writelines("collision sf node {} and node {}\n".format(p1.node.id, p2.node.id))
            return True
        if PRINT_ENABLED:
    ...
    def timing_collision(me: UplinkMessage, other: UplinkMessage):
        ...
        if other_end < critical_section_start or other.start_on_air > critical_section_end:
            ...
            # timing collision
            with open("colisao.txt", "a") as f:
                f.writelines('P1 has a critical section in [{} - {}]\n'.format(critical_section_start, critical_section_end))
        ...
        if me_end < critical_section_start or me.start_on_air > critical_section_end:
            ...
            # timing collision
            with open("colisao.txt", "a") as f:
                f.writelines('P2 has a critical section in [{} - {}]\n'.format(critical_section_start, critical_section_end))
    ...
    def power_collision(me: UplinkMessage, other: UplinkMessage, time_collided_nodes):
        ...
        with open("colisao.txt", "a") as f:
            ...
            f.writelines("pwr: node {0.node.id} {0.rss:3.2f} dBm node {1.node.id} {1.rss:3.2f} dBm; diff {2:3.2f} dBm\n".format(me,other, round(me.rss - other.rss,2)))

            if abs(me.rss - other.rss) < power_threshold:
                if PRINT_ENABLED:
                    print("collision pwr both node {} and node {} (too close to each other)".format(me.node.id,
                                                                                                    other.node.id))
                f.writelines("collision pwr both node {} and node {} (too close to each other)\n".format(me.node.id,
                                                                                                    other.node.id))
                ...
                if me in time_collided_nodes:
                    ...
                    f.writelines("collision pwr both node {} has collided by node {}\n".format(me.node.id, other.node.id))
                    me.collided = True
            else:
                ...
                if other in time_collided_nodes:
                    ...
                    f.writelines("collision pwr both node {} has collided by node {}\n".format(other.node.id, me.node.id))
                    other.collided = True
