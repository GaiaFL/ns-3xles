...
class Gateway:
    ...
    def packet_received(self, from_node, packet: UplinkMessage, now):
        ...
        self.packet_node[str(from_node.id)] = self.packet_node.get(str(from_node.id), 0) + 1
        ...
        possible_rx1, time_on_air_rx1, off_time_till_rx1 = self.check_duty_cycle(packet.payload_size, packet.lora_param.sf,
                                                                                 packet.lora_param.freq,
                                                                                 now)
        possible_rx2, time_on_air_rx2, off_time_till_rx2 = self.check_duty_cycle(packet.payload_size, LoRaParameters.RX_2_DEFAULT_SF,
